...
class DisplayBlogResult:
    ...
    def _handle_approved_click(self):
        ...
        # Get current feedback for approved content
        feedback_text = st.session_state.get('feedback', '').strip()
        
        st.session_state['feedback_result'] = ReviewFeedback(
            approved=True, 
            comments=feedback_text if feedback_text else "Content approved without additional comments"
        )
        st.session_state["feedback_submitted"] = True
        
        logger.info(f"Content approved with feedback: '{feedback_text}'")
    ...
    def _handle_revised_click(self):
        ...
        # FIX: Get and validate feedback properly
        feedback_text = st.session_state.get("feedback", "").strip()
        
        if not feedback_text:
            st.error("⚠️ Please enter revision comments before submitting!")
            logger.warning("Revision button clicked but no feedback provided")
            return
        ...
        logger.info(f"Revision requested with feedback: '{feedback_text}'")
    ...
    def process_graph_events(self, input_data=None):
        ...
        try:
            ...
            if st.session_state.current_stage not in ["complete", "feedback"]:
    ...
    def process_feedback(self):
        ...
        if st.session_state.get("generated_draft"):
            ...
            with st.expander("Stage 3: Feedback", expanded=True):
                # FIX: Use consistent session state key and immediate update
                feedback_text = st.text_input(
                    "Revision comments:",
                    placeholder="Please explain what changes you would like to see.",
                    key="revision_comments",
                    value=st.session_state.get("revision_comments", "")
                )
                
                # FIX: Update session state immediately when text changes
                st.session_state["feedback"] = feedback_text
                
                # DEBUG: Temporarily show what's being stored (remove this after testing)
                # st.write("🔍 DEBUG - Current feedback stored:", repr(st.session_state.get("feedback", "NONE")))
                
                col1, col2 = st.columns(2)
                with col1:
                    st.button("✅ Approve Content", 
                             on_click=self._handle_approved_click, 
                             key="blog_feedback_approve_button")
                with col2:
                    # FIX: Add validation before allowing submission
                    revision_disabled = not feedback_text.strip()
                    if revision_disabled:
                        st.warning("⚠️ Please enter revision comments before submitting.")
                    
                    st.button("Submit Revision Request", 
                             on_click=self._handle_revised_click,
                             key="blog_feedback_revise_button",
                             disabled=revision_disabled)
    ...
    def _download_blog_content(self, blog_content):
        ...
        if blog_content:
            ...
            href = f'<a href="data:file/markdown;base64,{b64}" download="{filename}">⬇️ Download Blog Content (Markdown)</a>'
    ...
    def handle_blog_workflow(self):
        ...
        # Stage 4: Process Feedback (Resume Graph)
            ...
            feedback_comment = st.session_state.get("feedback", "").strip()
            
            # FIX: Better validation of feedback comments
            if feedback_comment:  # Check for non-empty string
                ...
                logger.info(f"Resuming graph with feedback message: '{feedback_comment}'")
                
                # Clear feedback from session state to prevent reuse
